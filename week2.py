    ...
    while loop_num != True:
        ...
        if get_input.isdigit():
            if int(get_input) > 0 and int(get_input) < 10:
                ...
                print("Try Again Please. It seems like you have entered an invalid key.")
                loop_num = False
        else:
            print("Try Again.")
            loop_num = False
...
def game_over():
    greet = ["Thanks for playing!",
             "Good job!"]
    won = random.choice(greet)
    return won

def play_again():
    play_again = input("Do you want to play again ? Reply Y for yes / N or Enter Key for No : ")
    if play_again.lower() == "y":
        print("Lest do it! New Game Start")
        main()
    else:
